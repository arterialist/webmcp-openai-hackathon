    ...
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose() }}>
      <DialogContent className="compose-modal" showCloseButton={false} aria-describedby="compose-description" onCloseAutoFocus={(event) => { event.preventDefault(); onRestoreFocus() }}>
        <DialogHeader className="compose-header">
          <div>
            <div className="eyebrow"><Icon name="edit" size={14} /> {isEditing ? 'Edit article' : 'Article editor'}</div>
            <DialogTitle>{isEditing ? 'Edit article' : 'Write an article'}</DialogTitle>
            <DialogDescription id="compose-description">A complete article is available to you and browser agents through the same tools.</DialogDescription>
          </div>
          <DialogClose asChild>
        ...
        <form onSubmit={(event) => { event.preventDefault(); if (!canSubmit) return; onSubmit() }}>
          <div className="field-group">
            ...
            <Input id="compose-post-title" className="compose-title-input" value={draft.title} onChange={(event) => onDraftChange({ title: event.target.value })} placeholder="Article title" required />
          </div>
          <div className="field-group">
            <Label htmlFor="compose-post-excerpt">Short summary <span>(optional)</span></Label>
            <Textarea id="compose-post-excerpt" className="compose-excerpt-input" value={draft.excerpt} onChange={(event) => onDraftChange({ excerpt: event.target.value })} placeholder="A brief summary of what this covers." rows={3} />
          </div>
          <div className="field-group">
            <Label htmlFor="compose-post-body">Body</Label>
            <Textarea id="compose-post-body" className="compose-body-textarea" value={draft.body} onChange={(event) => onDraftChange({ body: event.target.value })} placeholder="Write your article here." rows={10} required />
